...
{{-- MOBILE OVERLAY & SIDEBAR (Tetap Dipertahankan) --}}
...
<aside id="mobileSidebar" class="fixed top-0 left-0 h-full w-[80%] max-w-[300px] bg-background/95 backdrop-blur-2xl border-r border-border z-[100] -translate-x-full pointer-events-none transition-transform duration-300 flex flex-col shadow-2xl">
    ...
    <nav class="p-6 flex flex-col gap-2 text-sm flex-1 overflow-y-auto">
        ...
        <div class="mt-8 pt-6 border-t border-border/50 space-y-4">
            ...
            @if (!session('is_login'))
                <a href="/login" class="w-full flex items-center justify-center gap-2 px-4 py-3 border border-primary text-primary font-mono text-xs font-bold uppercase tracking-widest hover:bg-primary hover:text-background transition-colors">
                    <i class="fa-solid fa-terminal"></i> SYS.LOGIN
                </a>
            @else
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-between px-4 py-3 border border-border text-muted font-mono text-xs uppercase tracking-widest hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-500 transition-colors group">
